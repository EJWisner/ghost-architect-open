...
export function generateBrief({ findings, ghostVersion, scanFile, codemaseRoot, tier }) {
...
<body>
  <header class="sticky">
    ...
    <div class="htitle">Ghost Brief™</div>
  ...
  <footer>
    ...
    <a href="https://ghostarchitect.dev" target="_blank" rel="noopener">ghostarchitect.dev</a>
  </footer>
</body>
</html>`;
}
...
export function writeBrief(brief, outputPath) {
  ...
  const htmlPath = path.join(path.dirname(jsonPath), 'ghost-brief.html');
  fs.writeFileSync(htmlPath, renderBriefHtml(brief), 'utf8');
