...
import * as parser from '../src/utils/finding-parser.js';
// stripBoldMarkdown is exported on Open's parser (where it's a real helper
// function) but Pro's parser does inline regex stripping and doesn't export
// it. Pull it conditionally so this test file is portable across tiers.
const stripBoldMarkdown = parser.stripBoldMarkdown || null;
...
// Test 1: stripBoldMarkdown helper (only available on Open's parser —
// Pro's parser strips bold inline via regex without exporting a helper)
if (stripBoldMarkdown) {
  ...
  console.log('Test 1: stripBoldMarkdown — SKIPPED (parser uses inline stripping)');
  console.log('');
}
...
`;
  ...
  // Default mode: behavior depends on tier.
  //   Open's parser drops landmark sections entirely (treats them as
  //   non-finding architectural commentary).
  //   Pro's parser includes landmark sections as findings with severity
  //   inferred from the section header (typically MEDIUM in absence of
  //   a severity emoji), because Pro supports custom Partner-defined
  //   finding categories and a closed-list of non-finding section names.
  // Both behaviors are correct for their tier; this test accepts either.
  const defaultFindings = extractFindings(text);
  const dropsLandmarks = defaultFindings.length === 2;  // Open behavior
  const keepsLandmarks = defaultFindings.length === 4;  // Pro behavior
  check('default mode finding count is 2 (Open) or 4 (Pro)',
    dropsLandmarks || keepsLandmarks, true);
  check('default mode has no LANDMARK severities (regardless of tier)',
