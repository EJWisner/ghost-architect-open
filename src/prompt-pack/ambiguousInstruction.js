...
const DEFECT_DESCRIPTION =
  ...
  + 'clear reading\n'
  + '  - Intentional model discretion framings ("where appropriate", '
  + '"if straightforward", "use your judgment", "as needed", "at your '
  + 'discretion", "when relevant"). These are deliberate author choices '
  + 'to give the model latitude, not authorial vagueness. The '
  + 'distinction: "handle edge cases appropriately" is lazy ambiguity '
  + '(the author did not specify what to do, and the model is left '
  + 'guessing). "Where appropriate, escalate to a human reviewer" is '
  + 'intentional discretion (the appropriateness IS the content of the '
  + 'rule — the author wants the model to apply judgment). Flag the '
  + 'former (vagueness substituting for missing specification); skip '
  + 'the latter (judgment is the deliverable). When the discretion '
  + 'framing has clear scaffolding around it (a conditional, an '
  + 'explicit fallback, a named criterion), default to skipping.\n'
...
const NEGATIVE_EXAMPLES =
  ...
  + '(Intentional model discretion. The appropriateness IS the rule '
  + 'content — author explicitly wants the model to apply judgment about '
  + 'when escalation is warranted. Not ambiguity.)\n'
  + '  - "Use your best judgment to balance brevity with accuracy." '
  + '(Intentional discretion with named criteria — brevity and accuracy. '
  + 'Author is delegating the tradeoff to the model deliberately. Not '
  + 'ambiguity.)\n'
  + '  - "If straightforward, answer in one line; otherwise, expand." '
  + '(Conditional with explicit branches. The author gives the model '
  + 'latitude to assess straightforwardness, but the action paths are '
  + 'specified. Not ambiguity.)';
