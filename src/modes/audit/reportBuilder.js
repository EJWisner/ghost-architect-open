...
export function buildAuditReport(results, meta = {}) {
  ...
  const branding = getBranding(meta.profile || null);
  ...
  // ── Methodology footer ───────────────────────────────────────────────
  sections.push(buildMethodologySection(branding));
...
function buildMethodologySection(branding) {
  return [
    '## Methodology',
    '',
    `This Inheritance Audit was generated by ${branding ? branding.companyName : 'Ghost Architect™'} in audit mode. The report combines four analyzers:`,
    '',
    ...
    '- **Hidden Dependency Map** enumerates direct dependencies and flags license risk and end-of-life status. License risk classification is available for npm dependencies in local directory scans. ZIP and GitHub scan paths fall back to "unknown" where the lockfile is not available. CVE scanning is not included in this report version; transitive dependency analysis is deferred to a future audit version.',
    ...
    '',
    `Report format version ${REPORT_VERSION}. Generated by ${branding ? branding.companyName : 'Ghost Architect™'}.`,
  ].join('\n');
}
